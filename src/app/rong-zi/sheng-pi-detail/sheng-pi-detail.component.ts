...
@Component({
  ...
  templateUrl: './sheng-pi-detail.component.html',
  styleUrls: ['./sheng-pi-detail.component.scss'],
})
// 审批详情
export class ShengPiDetailComponent extends DetailBasePage implements OnInit {
  ...
  private async getSignList() {
    const res = await this.request('/rongzi/signlist', {item_id: this.id});
  ...
  back() {
    ...
    this.setRequest("/rongzi/todoback", this.payload).then((res) => {
