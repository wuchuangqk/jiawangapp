export class HomeModel {
    public static itemList = [
        { icon: 'ios-notifications', color: '#7dc6ff', name: 'notice', title: '通知公告', url: '/notices/list', bage: '' },
        { icon: 'ios-bookmarks', color: '#7dc6ff', name: 'work-diary', title: '工作日志' , url: '/work_logs/list'},
        { icon: 'ios-paper', color: '#73d1d1', name: 'receive-document', title: '收文系统' },
        { icon: 'ios-mail-outline', color: '#fa7c92', name: 'FaWenXiTongPage', title: '发文系统' },
        { icon: 'ios-chatbubbles', color: '#7dc6ff', name: 'notice', title: '工作交流', url: '/work_dynamics/list' },
        { icon: 'ios-people', color: '#fbbd6d', name: 'JiaoBanDuBanPage', title: '交办督办督查' },
        { icon: 'ios-calendar-outline', color: '#b2d76a', name: 'QingJiaShenPiPage', title: '请假审批' },
        { icon: 'ios-alarm', color: '#c1a6f0', name: 'JiaBanShenPiPage', title: '加班审批' },
        { icon: 'md-pin', color: '#a3bdb9', name: 'WaiChuShenPiPage', title: '外出审批' },
        { icon: 'logo-twitch', color: '#6cd7ff', name: 'ZongHeShenPiPage', title: '综合审批' },
        { icon: 'ios-folder', color: '#6cd7ff', name: 'ZiChanGouZhiPage', title: '资产购置' },
        // { icon:"ios-briefcase-outline",color:"#b6c3b9", name:"", title:"项目库" },
        // { icon:"ios-create",color:"#b0b7bd", name:"XiangMuGuanLiPage", title:"项目管理" },
        // { icon:"ios-pricetags",color:"#89d4af", name: "TouZiBaoBiaoPage", title:"投资报表"},
        // { icon:"md-calendar",color:"#afa9b3", name:"JinDuBaoBiaoPage", title:"进度报表" },
        // { icon:"ios-calendar",color:"#75bcd8", name:"", title:"节点监控" },
        // { icon:"md-radio",color:"#fb8862", name:"JueCePingTaiPage", title:"决策平台" },
        // { icon:"ios-pin",color:"#94aac1", name:"QuanJingDiTuPage", title:"全景地图" },
        // { icon:"ios-school",color:"#babda7", name:"GongChengShenPiPage", title:"工程相关审批" },
        // { icon:"md-trending-up",color:"#6dbbff", name:"ZiChanTongJiPage", title:"资产统计" },
        // { icon:"logo-usd",color:"#f9a970", name:"RongZiHuanKuanPage", title:"融资还款" }
    ];
}
